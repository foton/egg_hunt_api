...
class ApplicationControllerTest < ActionController::TestCase
  ...
  test "authenticate user if token is send as username (for browsertesting)" do
    request.env['HTTP_AUTHORIZATION'] = ActionController::HttpAuthentication::Basic.encode_credentials(users(:bunny1).token, "blahblah")
    get :raise_test_exception
    assert_response :internal_server_error
    assert assigns(:current_user).present?
    assert_equal users(:bunny1), assigns(:current_user)
